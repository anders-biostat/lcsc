...
lc_vis <- function(cells, counts, pc_space, embedding, nn, k=50){
  ...
  # TODO: Change how this file is specified)
  ...
  # Select default marker gene.
  marker <- sample(rownames(counts),1)
  
  # Select default sample.
  sample_select <- sample_info$sample[1]
  
  # Define the subset based on the sample.
  ...
  nn_subset <- list(idx = (nn$idx[subset,] - correction), 
                    dists = nn$dist[subset,])
  
  # Get the average distance for cell_i to x nearest neighbors.
  n_NN <- 25
  avg_dist <- Matrix::rowMeans(nn_subset$dists[,2:(n_NN+1)])
  
  # Get the counts for the marker genes for all cells within the subset.
  c_M <- counts[marker, subset]
  
  # For each cell within the subset, get the sum of the counts for the marker 
  # gene considering all cells that are closer than the cutoff.
  counts_ci <- sapply(1:nrow(nn_subset$idx), function(cell_i){
    # Get the sum of counts of all neighbors
    sum(c_M[nn_subset$idx[cell_i,]])
  })
  
  # Get the total counts for all the cells in the sample
  c_Tot <- Matrix::colSums(counts[, subset])
  ...
  # Default gamma-correction ("square root")
  gamma = 0.5
  
  # Implement different methods to calculate the fraction
  f_M_c <- f_M^gamma
  
  # Default fraction threshold (very arbitrary!)
  threshold <- 0.02
  ...
  get_all_class_sample = function() {
    ...
    classification_matrix <- matrix(data=NA, nrow=sum(subset), ncol=1)
    rownames(classification_matrix) <- colnames(counts[,subset])
                  ...
                  if (value == 1) {
                    ...
                    rlc::lc_scatter(
                      ...
                      rlc::dat(
                        ...
                        y = embedding[subset,2],
                        label = colnames(counts)[subset],
                        colourValue = Matrix::colSums(counts[,subset]),
                        size = pt_size,
                        on_marked = function() {
                        }),
                      transitionDuration = 0,
  ...
  rlc::lc_input(type = "button", 
           ...
           on_click = function(value) {
             ...
             # Get the gloval environment (otherwise R cannot find the cell_types lists)
             g <- globalenv()
             ...
             if (!(current_class %in% names(g$annotations[[sample_select]]))) {
               ...
               g$annotations[[sample_select]][[current_class]] <- tibble::tibble(
                 barcode = colnames(counts[,subset])
               )
